...
public class MultiPlayerEasyActivity extends BaseGameActivity implements GameHelper.GameHelperListener,
    ...
    private final static int RC_SELECT_PLAYERS = 12345;

    // arbitrary request code for the waiting room UI.
    // This can be any integer that's unique in your Activity.
    private final static int RC_WAITING_ROOM = 12346;

    // at least 2 players required for our game
    private final static int MIN_PLAYERS = 2;

    private static final int MULTIPLAYER_EASY = 100;
    private static final char MESSAGE_UPDATE_POSITION = 'U';
    private static final char MESSAGE_WIN_GAME = 'W';
    ...
    protected void onActivityResult(int request, int response, Intent data) {
        ...
        if (request == RC_SELECT_PLAYERS) {
            ...
            loadingDialog = ProgressDialog.show(MultiPlayerEasyActivity.this, "",
                    "Please wait...", true);
            ...
            roomConfigBuilder.setVariant(MULTIPLAYER_EASY);
        ...
        if (request == RC_WAITING_ROOM) {
            if (response == Activity.RESULT_OK) {
                ...
                timer = new Timer();
                timer.scheduleAtFixedRate(new UnreliableMessageTask(), 0, 10);
    ...
    public void onRealTimeMessageReceived(RealTimeMessage realTimeMessage) {
        ...
        switch (buf[0]){
            case MESSAGE_UPDATE_POSITION:
                ...
                break;
            case MESSAGE_WIN_GAME:
                dialogShown = true;
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        AlertDialog.Builder builder = new AlertDialog.Builder(MultiPlayerEasyActivity.this);
...
// ===========================================================
    ...
    private void attachInitialElements() {
        ...
        obstacleFIFO.add(new SimpleObstacle(mCamera.getCenterX(), CAMERA_HEIGHT * 1.25f,
                CAMERA_WIDTH, CAMERA_HEIGHT / 2, CAMERA_WIDTH / 2
    ...
    private class UnreliableMessageTask extends TimerTask {
        @Override
        public void run() {
            byte[] code = {MESSAGE_UPDATE_POSITION};
            float x = mainShip.getShipMesh().getX();
            float y = mainShip.getShipMesh().getY();
            float rotation = mainShip.getRotation();

            byte[] positionMessage = concatByteArrays(code, concatByteArrays(float2ByteArray(x),
